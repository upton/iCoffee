...
public class ShardedJedisSentinelPool extends Pool<ShardedJedis> {
    ...
    private List<HostAndPort> initSentinels(Set<String> sentinels, final List<String> masters) {
        ...
        if (masters.size() == shardMasters.size()) {
            ...
            throw new JedisConnectionException("Not all shards master can be accessed. Except " + masters.size() + " masters, but "
                    + shardMasters.size() + " masters can be accessed");
        }
     ...
     */
    private class ShardedJedisFactory implements PooledObjectFactory<ShardedJedis> {
